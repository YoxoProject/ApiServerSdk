...
class CountrySettingsEntry(BaseModel):
    """
    Paramètres du pays
    """ # noqa: E501
    invitation_needed: Optional[StrictBool] = Field(default=None, description="Invitation obligatoire pour rejoindre le pays", alias="invitationNeeded")
    recruitment_open: Optional[StrictBool] = Field(default=None, description="Pays ouvert aux recrutements", alias="recruitmentOpen")
    do_kick_recruit_war_reason: Optional[StrictBool] = Field(default=None, description="Expulsion automatique des recrues lorsque qu'elles donnent une raison de guerre", alias="doKickRecruitWarReason")
    do_kick_member_negative_power: Optional[StrictBool] = Field(default=None, description="Expulsion automatique des membres lorsque leur power devient négatif", alias="doKickMemberNegativePower")
    permissions: Optional[List[PermissionEntry]] = Field(default=None, description="Permissions accordées dans le pays")
    logs: Optional[List[SettingLogEntry]] = Field(default=None, description="Logs de pays")
    __properties: ClassVar[List[str]] = ["invitationNeeded", "recruitmentOpen", "doKickRecruitWarReason", "doKickMemberNegativePower", "permissions", "logs"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "invitationNeeded": obj.get("invitationNeeded"),
            "recruitmentOpen": obj.get("recruitmentOpen"),
            "doKickRecruitWarReason": obj.get("doKickRecruitWarReason"),
            "doKickMemberNegativePower": obj.get("doKickMemberNegativePower"),
            "permissions": [PermissionEntry.from_dict(_item) for _item in obj["permissions"]] if obj.get("permissions") is not None else None,
            "logs": [SettingLogEntry.from_dict(_item) for _item in obj["logs"]] if obj.get("logs") is not None else None
        })
        return _obj
